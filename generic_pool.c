...
pool_free_pool(generic_pool_t * const pool)
{
    ...
    while ((dfb = fb_list_extract_head(&pool->free_fbs)) != NULL) {
		...
		pool->callback_fns.delete_thing_fn(pool, pool->callback_v, dfb);
        pthread_mutex_lock(&pool->lock);
    }
    pthread_mutex_unlock(&pool->lock);
}
...
void *
generic_pool_fb_new(generic_pool_t * const pool, ...)
{
    struct generic_h * dfb;
    ...
    while (slot != NULL) {
		...
		va_copy(a2, args);
		score = pool->callback_fns.try_reuse_thing_fn(pool, pool->callback_v, slot->fb, a2);
		va_end(a2);
    ...
    if ((dfb = pool->callback_fns.alloc_thing_fn(pool, pool->callback_v, args)) == NULL) {
        pthread_mutex_lock(&pool->lock);
        --pool->fb_count;
        goto fail_unlock;
    }
